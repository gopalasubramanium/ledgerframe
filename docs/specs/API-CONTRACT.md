...
| **reshape ✅** | `GET /api/v1/portfolio/holdings` (+ `?account_id=`) | **§9-11 / Amendment G** | **Delivered 2026-07-16 (Phase 0 commit 9).** `account_id` scopes the SAME canonical reader to one account's holdings (the account rollup's drill-down target) — the `?symbol` filter-not-recompute posture; each `HoldingValue` already carries `account_id`. Unknown id → honest empty list. **Phase 0 ships the reader param ONLY**; the Holdings-PAGE URL filter + clearable chip is Phase-1 work (Amendment G, dated delta note in `page-holdings.md`). |
...
| **reshape ✅** | `GET /api/v1/accounts` (+ per-account `value_display`, envelope `total_display`) | **D-105 / §9-10** | **Delivered 2026-07-16 (Phase 0 commit 8).** Money on the page is a **served display string** via the platform path (`format_money_display`) — the whole-unit `_f` rounding is gone (`value`/`total` are now 2dp). The rollup is in BASE currency, so it is bare (§12in-1); an account's native holdings currencies ride the existing `currencies` chips. `base_currency` was already in the envelope. Route is untyped `-> dict` (added fields not stripped) → no contract shape change; pinned by a test. |
| **behaviour ✅** | `POST/PATCH /api/v1/accounts` — out-of-vocab `kind`/`currency` → **400** | **§9-9** | **Delivered 2026-07-16 (Phase 0 commit 7).** An invalid `kind` **silently coerced to brokerage** and an unsupported `currency` was `upper()[:3]`-passed (the A9 free-text-enum gap). Now both are honest **400s** (the `policy_status`/`base_currency` precedent); plain-language details, no decision IDs. ⚠ Invisible to a shape check — pinned by tests. |
...
| **add ✅** | `POST /api/v1/entities`, `PATCH /api/v1/entities/{eid}`, `DELETE /api/v1/entities/{eid}` | **D-065 / §9-6 / Amendment H** | **Delivered 2026-07-16 (Phase 0 commit 6).** Entity CRUD (name + `kind` from `entity_kind`, **vocab-enforced → 400**). `ENTITY_KINDS` single-sourced (`services/entities.py`) and served by `/refdata` (the `policy_status` graduation, Amendment H). **DELETE blocked** while any account references the entity — honest **400**, service-level (no cascade; the FK has no `ondelete`). §9-7 / D-029: **no "Household" special-casing** — the lowest-id entity is renamable/re-kindable/deletable under the same guard (no `is_default`). +1 path → **130**. |
| **add ✅** | `GET /reports/pack` → **`HTMLResponse`** | **D-038 / D-061** (reports-pack §3b, Pack milestone) | **Delivered 2026-07-17 (Pack Phase 0).** The one sanctioned print/export artifact (NOT under `/api/v1` — a backend-served HTML document at the bare path, registered in `main.py` **before** the SPA catch-all, IN the OpenAPI schema). Composes the canonical readers server-side (net-worth trend · review · cash flow · scenarios consolidated; net worth · drift · realised · risk + attribution per entity via `entity_id`, §9-2). **f-string composition, no Jinja2 → no new dependency.** Self-contained (inline CSS, no app JS, no external fetch — Pack-9). Access follows the platform read posture (`require_read_auth`, the router-wide read gate) — **no bespoke one-route guard**. +1 path → **132**. |
...
| **reshape ✅** | `POST /api/v1/portfolio/purge-deleted` (+ `PinConfirm` request body) | **§14dr-20 / D-103** | **Delivered 2026-07-18.** D-103 was **violated**: the ConfirmDialog PIN was collected then discarded, and `require_pin` authorised on the **ambient session** (`token_is_valid`), never a fresh PIN. Now the irreversible purge verifies a **freshly-entered PIN** server-side (`verify_fresh_pin`) — the session token is deliberately not accepted in its place (SECURITY-BASELINE §3). Visible request-body shape change, **no path added/removed (134 held)**. `require_pin` still runs first (API tokens / no-PIN installs refused). Fresh-PIN pin: wrong PIN → 401, correct → 200. |
| **reshape ✅** | `POST /api/v1/system/reset-data` (+ `PinConfirm` request body) | **§14dr-20 / D-103** | **Delivered 2026-07-18.** Same D-103 enforcement applied **consistently** to the sibling wipe — the fresh PIN is verified server-side, the ambient session no longer satisfies it. Visible request-body shape change, **no path added/removed (134 held)**. |
...
| **allow-list ✅** | `GET`/`PUT /api/v1/settings` — **`long_term_days`** ADDED (numeric; validator mirrors the route's `ge=0, le=3660`). Resolved as the realised-gains / tax-lots default in **ONE place** (`tax.resolve_long_term_days`); an explicit `?long_term_days` **wins** (PARAM-WINS) | **page-settings §9-1** (Amendment A / D-077 / Guarantee 4) | Server-persisted **neutral** threshold, **no jurisdiction presets**. ⚠ The allow-list key itself is **INVISIBLE to a shape check** (free dict) — pinned by **served-value + validator tests**. *(Separately: PARAM-WINS flipped the four `long_term_days` **query params** to `default=None` → `anyOf:[integer,null]` — a visible param-schema change **regenerated same-commit in delta 2**, no path added/removed.)* |
| **retired** | `PUT /api/v1/settings` — ~~**`rotation_seconds`**~~ **REMOVED 2026-07-18** | **page-settings §9-2(b)** (D-078 / D-044) | Write-only, no consumer. Rotation **parked to R-37** — re-added spec-first **with its engine**. Unknown-key-400 test pins it. |
| **retired** | `PUT /api/v1/settings` — ~~**`rotation_pages`**~~ **REMOVED 2026-07-18** | **page-settings §9-2(b)** (D-078 / D-044) | Write-only, no consumer. Rotation **parked to R-37**. Unknown-key-400 test pins it. |
| **retired** | `PUT /api/v1/settings` — ~~**`focus_page`**~~ **REMOVED 2026-07-18** | **page-settings §9-2(b)** (D-078 / D-044) | Write-only, no consumer. Rotation **parked to R-37**. Unknown-key-400 test pins it. |
| **retired** | `PUT /api/v1/settings` — ~~**`reduced_motion`**~~ **REMOVED 2026-07-18** | **page-settings §9-6** (D-078) | Redundant server row — **per-device localStorage is its only home** (DisplayProvider). Unknown-key-400 test pins it. |
| **retired** | `PUT /api/v1/settings` — ~~**`high_contrast`**~~ **REMOVED 2026-07-18** | **page-settings §9-6** (D-078) | Redundant server row — **per-device localStorage is its only home** (DisplayProvider). Unknown-key-400 test pins it. |
| **retired** | `PUT /api/v1/settings` — ~~**`refresh_interval_seconds`**~~ **REMOVED 2026-07-18** | **page-settings §9-7** (D-078) | Write-only, **no consumer and no spec home**. Re-add spec-first if a feature ever names it. Unknown-key-400 test pins it. |
| **retired** | `PUT /api/v1/settings` — ~~**`display_sleep_minutes`**~~ **REMOVED 2026-07-18** | **page-settings §9-7** (D-078) | Write-only, **no consumer and no spec home**. Re-add spec-first if a kiosk feature ever names it. Unknown-key-400 test pins it. |
